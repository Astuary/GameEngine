...
const float PT = 3.14159265f;

namespace {

const char* VERT_SRC = R"(#version 440
...
const char* FRAG_SRC = R"(#version 440
...
void DebugRenderer::drawBox(const glm::vec4& destRect, const Bengine::ColorRGBA8& color, float angle){
    ...
    m_verts[i + 2].position = rotatePoint(tr, angle) + halfDims + positionOffset;
    m_verts[i + 3].position = rotatePoint(br, angle) + halfDims + positionOffset;
...
void DebugRenderer::drawCircle(const glm::vec2& centre, const Bengine::ColorRGBA8& color, float radius){
    static const int NUM_VERTS = 100;
    int start = m_verts.size();

    for(int i = 0; i < NUM_VERTS; i++){
        float angle = ((float)i / NUM_VERTS) * PI * 2.0f;
        m_verts[start + i].position.x = cos(angle) * radius;
        m_verts[start + i].position.y = sin(angle) * radius;
